...
export class NanoAccessibility extends BaseHTMLElement {
  ...
  constructor() {
    ...
    this.sessionId =
      (typeof localStorage !== "undefined" && localStorage.getItem("nanoAccessibilitySessionId")) ||
      (typeof crypto !== "undefined" && crypto.randomUUID && crypto.randomUUID()) ||
      Math.random().toString(36).slice(2);

    if (typeof localStorage !== "undefined") {
      localStorage.setItem("nanoAccessibilitySessionId", this.sessionId);
    }
  }
}
...
if (typeof window !== "undefined" && window.customElements) {
  customElements.define("nano-accessibility", NanoAccessibility);
}